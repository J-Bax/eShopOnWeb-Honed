...
public class CatalogItemListPagedEndpoint : IEndpoint<IResult, ListPagedCatalogItemRequest, IRepository<CatalogItem>>
{
    ...
    public async Task<IResult> HandleAsync(ListPagedCatalogItemRequest request, IRepository<CatalogItem> itemRepository)
    {
        ...
        // When the page is not full, we know the total without a separate COUNT query.
        // Only fall back to CountAsync when the page is full and there may be more items.
        int totalItems;
        int skip = request.PageIndex * request.PageSize;
        if (items.Count < request.PageSize)
        {
            totalItems = skip + items.Count;
        }
        else
        {
